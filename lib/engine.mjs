...
export async function runFetch(cfg, state, opts = {}) {
  ...
  for (const e of entries) {
    ...
    if (gotText) {
      ...
      // no circuit breaker: gap-heavy stretches (whole shows a source lacks)
      // are normal and must not stop the sweep — items missed today are
      // stamped and skipped going forward, so every run still progresses
    }
