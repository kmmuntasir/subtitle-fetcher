...
import { BASE, sleep, keyOf, reconstructPath, trueVideoPath } from "./utils.mjs";
...
/** rolling pace of the active run, read by the service for the countdown:
 *  lastAt = when the last item finished, avgMs = smoothed gap between items */
export const fetchPace = { lastAt: 0, avgMs: 0 };
...
export async function runFetch(cfg, state, opts = {}) {
  const { limit = cfg.maxPerRun ?? null, onlySub = "", rescan = false,
          ...
          takePriority = null, saveState = null, skipRescan = false,
          providers = null, skipA7MissedToday = false, noPark = false } = opts;
  const started = Date.now();

  // providers before refresh so a missing config fails fast
  const provs = buildProviders(cfg, state)
    .filter(p => !providers || providers.includes(p.id));
  if (!provs.length) {
  ...
  let entries = Object.entries(state.files)
    ...
    // a7-only sweeps skip titles addic7ed already returned zero results for
    // today — each cycle then makes strict forward progress past gap blocks
    .filter(([, r]) => !(skipA7MissedToday && r.a7miss === utcDay()))
    .map(([k, r]) => ({ k, ...r }));
  // try fresh items before re-burning attempts on older failures — each run
  // then maximizes new downloads per attempt spent (stable sort keeps scan
  // order within the same attempt count)
  entries.sort((a, b) => (a.attempts ?? 0) - (b.attempts ?? 0));
  ...
  let missStreak = 0;                    // consecutive misses with zero downloads
  ...
  for (const e of entries) {
    ...
    // resolve the video's true on-disk casing — players pair sidecar subs by
    // exact filename, and the state key is lowercased
    const vPath = trueVideoPath(reconstructPath(e.k, paths));
    ...
    let gotText = null, usedProv = null, candInfo = null, lastFailNote = "", a7Empty = false;
    ...
    for (const prov of provs) {
      ...
      try {
        ...
        if (prov.id === "a7") a7Empty = cands.length === 0;
    ...
    if (gotText) {
      ...
      const rec = state.files[e.k] ?? {};
      // a7-only sweeps must not burn strikes toward parking — sd never got a
      // shot at the item; the a7miss stamp already excludes it from today's
      // sweeps and the daily full chain judges it fairly
      if (!noPark) rec.attempts = (rec.attempts ?? 0) + 1;
      rec.lastError = lastFailNote || "no provider had it";
      if (a7Empty) rec.a7miss = utcDay();      // today's a7-only sweeps skip this
      rec.when = new Date().toISOString();
      rec.status = !noPark && rec.attempts >= cfg.attemptsBeforePark ? "parked" : "failed";
      ...
      // catastrophic guard only: gap-heavy libraries legitimately produce long
      // miss stretches (titles simply absent from a source), so trip solely
      // when a run has downloaded NOTHING at all — i.e. the source itself is
      // dead/degraded — and not on ordinary gaps
      if (++missStreak >= 100 && stats.done === 0) {
        log(YEL("⏸ 100 misses with zero downloads this run — source looks unavailable; ending to protect the queue. Retries after backoff."));
        onEvent({ ev: "streak_stop", misses: missStreak });
        stopped = true;
        break;
      }
    }
    ...
    // per-item cadence (EMA of completion gaps) for the UI countdown — long
    // gaps (deferral stretches, idle runs) refresh the anchor without
    // polluting the average
    {
      const now = Date.now();
      const gap = now - fetchPace.lastAt;
      if (fetchPace.lastAt && gap < 60e3) fetchPace.avgMs = fetchPace.avgMs ? Math.round(fetchPace.avgMs * 0.7 + gap * 0.3) : gap;
      fetchPace.lastAt = now;
    }
  }
  ...
  function finish(exitCode) {
    opts.saveState?.();   // every run persists its final state — the %5 cadence
                          // above only covers mid-run crash tolerance, and never
                          // fires for short/limited runs (e.g. single-item minis)
