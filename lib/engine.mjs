...
export async function runFetch(cfg, state, opts = {}) {
  const { limit = cfg.maxPerRun ?? null, onlySub = "", rescan = false,
          log = console.log, onEvent = noop, shouldStop = noop } = opts;
  ...
  if (rescan || !state.scannedAt || Date.now() - state.scannedAt > 20 * 3600e3) {
  ...
  outer:
  for (const e of entries) { // eslint-disable-line no-labels
  ...
  function finish(exitCode) {
    ...
    onEvent({ ev: "run_end", done: stats.done, missed: stats.missed, minutes: +mins, stopped });
