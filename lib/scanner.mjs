...
export function refreshInventory(cfg, state, { silent = false, progress } = {}) {
  ...
  for (const it of items) {
    ...
    if (p.coverMtime !== it.mtime) {
      if (missing && (p.status === "done" || p.status === "covered")) {
        ...
        else if (!missing && p.status !== "done" && p.status !== "covered") { p.status = "covered"; credited++; }
      }
  ...
  if (credited) console.log(`scan: credited ${credited} locally-added subtitle(s) (manual drops)`);
  return items;
}
...
export async function refreshInventoryAsync(cfg, state, { onProgress, onWalk } = {}) {
  ...
  for (let i = 0; i < uniq.length; i++) {
    ...
    else if (!missing && p.status !== "done" && p.status !== "covered") {
      // a subtitle appeared without the engine downloading it (manual drop,
      // player-side fetch) — credit the record so it leaves the queue
      p.status = "covered"; credited++;
    }
  ...
  if (credited) console.log(`scan: credited ${credited} locally-added subtitle(s) (manual drops)`);
  return { total: uniq.length, errors: scanErrors, renamed: renames, credited };
}
