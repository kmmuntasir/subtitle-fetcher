...
export function cleanShowName(rawSeg) {
  ...
  t = t.replace(/[._]+(?=\S)/g, " ");
  for (let i = 0; i < 4; i++) {
    // "... - Complete Series"/"- Full Season" suffixes: keep the title before the dash
    const cm = /^(.*?)[\s._-]*-[\s._-]*(?:complete|full)\b/i.exec(t);
    if (cm && cm[1].trim()) t = cm[1];
  ...
  // collection-word suffixes ("Breaking Bad Complete", "Show Full") — only when
  // a real title remains; these folders poison provider search terms otherwise
  const beforeColl = t.replace(/\s+\b(?:complete|completa|complet|full)\b.*$/i, "").trim();
  if (beforeColl.split(/\s+/).length >= 2) t = beforeColl;
  // trailing bare year ("Avatar The Last Airbender 2005") — same poisoning;
  // genuine year-titled shows ("1923") survive via the ≥2-word guard
  const beforeYear = t.replace(/\s+\b(?:19|20)\d{2}\b\s*$/i, "").trim();
  if (beforeYear.split(/\s+/).length >= 2) t = beforeYear;
